    ...
    def _open_help(self, category=None):
        ...
        sections = {
            "操作指南": [
                ...
                ("前进与后退", [
                    ("p", "· Alt + ← 返回上一篇笔记，Alt + → 再前进一篇。"),
                    ("p", "· 也可以直接点内容区左上角的 ← / → 箭头。"),
                    ("p", "· 适用于内部链接、反向链接、搜索结果、图片链接等各类跳转，"),
                    ("p", "  误触跳转时可以快速回到原来的位置。"),
                    ("p", "· 箭头变灰表示该方向没有可返回 / 前进的笔记。"),
                ]),
                ("搜索", [
            ...
            "笔记格式": [
                ...
                ("图片", [
                    ("code", "![说明文字](图片文件名.png)"),
                    ("p", "· 图片放在笔记所在目录即可直接写文件名；也支持 data/ 内的"),
                    ("p", "  相对路径（如 images/a.png）。"),
                    ("p", "· 默认「适应宽度」：比正文宽时等比缩小，比正文窄时保持原样"),
                    ("p", "  （不放大、不拉伸）。"),
                    ("p", "· 视图菜单可切到「原始尺寸」：按图片本身像素显示，超宽时用内容"),
                    ("p", "  区下方的水平滚动条查看。"),
                    ("p", "· 鼠标移到图片上，状态栏会显示它的说明文字；说明写在地址后面"),
                    ("p", "  的引号里（没有就显示方括号里的文字）："),
                    ("code", "![备用文字](a.png \"鼠标悬浮时显示的文字\")"),
                    ("p", "· 用链接把图片包起来，图片就可以点击："),
                    ("code", "[![图](a.png)](笔记名)\n"
                             "[![图](a.png)](https://example.com)"),
                    ("p", "· 单击跳转到站内笔记，Ctrl + 单击用浏览器打开网址；"),
                    ("p", "  鼠标悬浮时状态栏会提示这张图指向哪里。"),
                    ("p", "· 网络图片（http:// 或 https:// 开头）不会被加载，会显示占位文字。"),
                ]),
                ("支持的 Markdown 语法", [
